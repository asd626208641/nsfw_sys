...
<script type="text/javascript">
	function doVerify() {
		...
		if (account != "") {
			$.ajax({
				//2.校验
				url : "${bathPath}nsfw/user_verifyAccount.action",
				date : {"user.account":account},
				type : "post",
				success : function(msg) {
					if ("true" != msg) //说明返回值是其他，不是true，说明存在相同的account
						//账号存在
						alert("账号已经存在，请输入其他账号");
				}
			});
		}
	}
...
<body class="rightBody">
	<form id="form" name="form" action="${basePath}nsfw/user_add.action"
					...
					<table id="baseInfo" width="100%" align="center" class="list"
						...
						<tr>
							...
							<td><s:textfield id="birthday" name="user.birthday"
									readonly="true"
									onfocus="WdatePicker({'skin':'whyGreen','dataFmt':'yyyy--MM--dd'});" /></td>
						</tr>
						<tr>
